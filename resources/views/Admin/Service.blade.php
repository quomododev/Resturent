									...
									<div class="row mg-top-30">
                                        <div class="col-12 sherah-flex-between">
                                            ...
                                            <div class="sherah-breadcrumb">
                                                <h2 class="sherah-breadcrumb__title">Services</h2>
                                            ...
                                            <a  href="{{route('Service.create')}}" data-mo class="sherah-btn sherah-gbcolor">Create Service</a>
                                        </div>
                                    </div>
									<div class="sherah-table sherah-page-inner sherah-border sherah-default-bg mg-top-25">
                                        <table id="ServiceTable" class="sherah-table__main sherah-table__main-v3">
                                            <!-- sherah Table Head -->
                                            <thead class="sherah-table__head">
                                                <tr>
                                                    ...
                                                    <th class="sherah-table__column-3 sherah-table__h3">Color</th>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="sherah-table__column-2 sherah-table__data-2">
														<div class="sherah-table__product-content">
															<p class="sherah-table__product-desc">{{$service['title']}}</p>
														</div>
                                                    </td>
													<td class="sherah-table__column-2 sherah-table__data-2">
														<div class="sherah-table__product-content">
															<p class="sherah-table__product-desc">{!! $service['description'] !!}</p>
														</div>
                                                    </td>
                                                     <td class="sherah-table__column-5 sherah-table__data-5">
														<div class="sherah-table__product-content">
															<p class="sherah-table__product-desc"><img src="{{asset($service['icon'])}}" alt=""></i></p>
														</div>
                                                    </td>
													<td class="sherah-table__column-5 sherah-table__data-5">
														<div class="sherah-table__product-content">
																<div class="color-circle" style="background-color: rgb({{$service->red}}, {{$service->green}}, {{$service->blue}});"></div>
														</div>
                                                    </td>
													@if($service->status == 1)
													<td class="sherah-table__column-6 sherah-table__data-6">
														<div class="sherah-ptabs__notify-switch sherah-ptabs__notify-switch--two">
															<label class="sherah__item-switch">
																  <input id="status" onclick="changeStatus({{ $service->id }})" type="checkbox" checked="">
												  ...
												  <td class="sherah-table__column-6 sherah-table__data-6">
														<div class="sherah-ptabs__notify-switch sherah-ptabs__notify-switch--two">
															<label class="sherah__item-switch">
																<input onclick="changeStatus({{ $service->id }})" type="checkbox">
																<span class="sherah__item-switch--slide sherah__item-switch--round"></span>
															</label>
														</div>
													</td>
												  @endif
													<td class="sherah-table__column-8 sherah-table__data-8">
														<div class="sherah-table__status__group">
															<a href="{{route('Service.edit',$service->id)}}" class="sherah-table__action sherah-color2 sherah-color3__bg--opactity OurService">
                                                            ...
                                                            <form action="{{route('Service.destroy',$service['id'])}}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="button" style="border:none" onclick="confirmation(event)" class="sherah-table__action sherah-color2 sherah-color2__bg--offset">
...
<script>
    ...
    $.ajax({
        ...
        url: "{{ route('chnage.service.status', ':Id') }}".replace(':Id',Id),
        dataType: "json",
...
<script> 
   $(document).ready(function(){
    $('#ServiceTable').on('click','.OurService',function(){
        var Id = $(this).attr('data-id');
        console.log(Id);
        $.ajax({
        type:"GET",
        data: $('#OurMemberTbl').serialize(),
        url: "{{ route('Service.edit',':Id') }}".replace(':Id',Id),
        dataType: "json",
        success: function(response){
        if(response.status == true)
        {
            ...
            CKEDITOR.instances.service_description2.setData(response.service.description);
            $('#image').attr('src', response.service.icon);
            $('#color').val(response.service.color);
        }
        }
    });
    });       
