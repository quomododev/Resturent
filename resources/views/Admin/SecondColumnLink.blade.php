							...
							<div class="sherah-body">
								...
								<div class="sherah-dsinner">
									<div class="row mg-top-30">
                                        <div class="col-12 sherah-flex-between">
                                            ...
                                            <div class="sherah-breadcrumb">
                                                <h2 class="sherah-breadcrumb__title">Footer Second Column Link</h2>
                                                <ul class="sherah-breadcrumb__list"> 
                                                    <li><a href="#">Home</a></li>
                                                    <li class="active"><a href="order-list.html">Footer Second Column Link</a></li>
                                                </ul>
                                            </div>
									...
									<div class="sherah-table sherah-page-inner sherah-border sherah-default-bg mg-top-25">
                                        <table id="sherah-table__vendor " class="sherah-table__main sherah-table__main-v3 second_column">
                                            <!-- sherah Table Head -->
                                            ...
                                            <tbody class="sherah-table__body">
												@foreach($second_column_link as $index => $link)
                                                <tr>
													...
													<td class="sherah-table__column-8 sherah-table__data-8">
														<div class="sherah-table__status__group">
															<a data-id="{{$link->id}}" data-bs-toggle="modal" data-bs-target="#EditColumnLinkModal" class="sherah-table__action sherah-color2 sherah-color3__bg--opactity SecondColumn">
															...
															<a href="{{route('delete.footer.second.column.link',$link['id'])}}" onclick="confirmation(event)" class="sherah-table__action sherah-color2 sherah-color2__bg--offset">
									...
									<div class="row mg-top-40">
										<div class="sherah-pagination">
											<ul class="sherah-pagination__list">
												<li>{{$second_column_link->links()}}</li>
											</ul>
										</div>
									</div>
								</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Create Footer Second Column Link</h5>
      ...
      <form class="sherah-wc__form-main" action="{{route('store.footer.second.column.link')}}" method="POST" enctype= "multipart/form-data" >
                @csrf
            <div class="variant">
                    <div class="row">
...
<div class="modal fade" id="EditColumnLinkModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Create Footer First Column Link</h5>
        <button type="button" style="background-color:white;color:#6176FE;" class="btn btn-primary" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
	...
	$(document).ready(function(){
		$(".second_column").on('click','.SecondColumn',function(){
			var Id = $(this).attr('data-id');
            console.log(Id);
			if(Id){
             $.ajax({
                 url: "{{ route('edit.footer.second.column.link',':Id') }}".replace(':Id',Id),
                 dataType: 'json',
		...
		$("#Update_First_column").on('submit',function(event)
		{
			...
			type:"POST",
			data: formdata,
			url: "{{ route('update.footer.second.column.link', ':Id') }}".replace(':Id',Id),
			dataType: "json",
			success: function(response){
